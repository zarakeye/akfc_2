...
export default function Cropper({ onCrop, onChangeAll, onClear }: CropperProps) {
      ...
      <div className="flex flex-wrap gap-4 mt-4">
        {images.map((img, i) =>
          currentIndex === i ? null : (
            ...
            //   }}
            // >
            <div key={img.id} className="relative w-32 h-32 border rounded-md overflow-hidden">
              <Image
                src={img.preview}
                alt={img.file?.name ?? `image-${i}`}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (onChangeAll) {
                    const newImgs = images.filter((_, idx) => idx !== i).map((img) => ({ name: img.file.name, preview: img.preview }));
                    onChangeAll(newImgs);
                  }
                }}
              ...
              <input
                ...
                onChange={() => {
                  setImages((prev) => {
                    ...
                    if (onChangeAll) {
                      onChangeAll(next.map((img) => ({ name: img.file.name, preview: img.preview })));
                    }
                    return next;
                  });
                }}
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="origin-center" style={{ transform: `scale(${zoom}) rotate(${rotate}deg)` }}>
                  <Image src={images[currentIndex].preview} alt="to crop" className="max-w-[600px] max-h-[450px] pointer-events-none" draggable={false} width={600} height={450} />
                </div>
              </div>
