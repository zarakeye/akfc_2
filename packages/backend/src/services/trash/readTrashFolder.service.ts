...
import type {
  ...
  TrashFolderNode,
  TrashMeta,
} from "packages/contracts/src/trash/trash-node.types";

import { normalizePath } from "./trash.utils";
    ...
    do {
      ...
      for (const asset of res.resources ?? []) {
        out.push({
          ...
          url: asset.secure_url as string,
...
export async function readTrashFolder(params: {
  ...
  const children = computeDirectChildren({ assets, storagePrefix, virtualPrefix, meta });
