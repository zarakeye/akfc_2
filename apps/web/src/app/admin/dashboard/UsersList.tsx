...
export default function UsersList(): JSX.Element {
  ...
  const columns: Column<User>[] = [
    ...
    { property: 'email', displayName: 'Email', type: 'string' },
    {
      ...
      displayName: 'Rôle',
      type: 'string',
      // NOTE: la lib ne type pas toujours `row` dans la signature -> cast minimal.
      render: ((roleId: unknown, row: unknown) => {
        const u = row as User;
        return (
          <UpdateUserRoleById
            userId={u.id}
            currentRoleId={(roleId as number | null) ?? null}
            roles={roles}
            onChanged={() => usersQuery.refetch()}
          />
        );
      }) as Column<User>['render'],
    },
