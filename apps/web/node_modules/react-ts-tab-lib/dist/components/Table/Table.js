 ...
 */
const generatePagesNumbers = (current, total) => {
    if (total <= 1)
        return [];
...
function Table({ columns = [], rows = [], onRowHover, onRowClick, styleClassNames, rangeLengthOptions = [10, 20, 50, 100], defaultOrder, textContent }) {
    const [rangeLength, setRangeLength] = useState(rangeLengthOptions[0]); // Max number of displayed rows at once
    const [allRows, setAllRows] = useState(rows);
    const [currentPage, setCurrentPage] = useState(1);
    const [pagesNumber, setPagesNumber] = useState(1);
    const [displayedRange, setDisplayedRange] = useState([]); // Rows currently displayed (on the current page)
     ...
     */
    const handleOrderChange = (e, property) => {
        e.preventDefault();
     ...
     */
    const handleRangeLengthChange = (value) => {
    ...
    const handleSearchInput = (e) => {
        const rowsMatchingWithSearchInput = [];
    ...
    const pagesNumbers = useMemo(() => generatePagesNumbers(currentPage, pagesNumber), [currentPage, pagesNumber]);
    const rangeLengthOptionsTags = rangeLengthOptions.map((option) => ({
        value: option,
        label: _jsx("span", { className: `
                ...
                `, children: `${textContent?.sampleLabelPrefix ?? 'Show '} ${option} ${textContent?.sampleLabelSuffix ?? ' entries'}` }, option)
    }));
    useEffect(() => {
    ...
    return (_jsx("div", { className: 'table-root', children: _jsxs("div", { className: `my-5`, children: [_jsxs("div", { className: `flex flex-col justify-between items-center my-5 gap-y-3.5 ${styleClassNames?.rangeOptionsAndSearchBarArea ?? ''}`, children: [_jsxs("div", { children: [_jsx("label", { htmlFor: "rangeLength", className: 'sr-only', children: "Displayed entries" }), _jsxs(Dropdown, { children: [_jsx(DropdownTrigger, { children: _jsx(Button, { variant: 'bordered', className: `
                    ...
                    ${styleClassNames?.rangeLengthOptions?.buttonPadding ?? 'px-[10px]'}`, children: _jsxs("span", { children: [`${textContent?.sampleLabelPrefix ?? 'Show'} ${selectedValue} ${textContent?.sampleLabelSuffix ?? 'entries'}`, _jsx(ChevronDownIcon, { className: "pl-[10px] inline-block h-5 w-[30px] text-white ui-open:rotate-180 transition-transform", "aria-hidden": "true" })] }) }) }), _jsx(DropdownMenu, { disallowEmptySelection: true, "aria-labelledby": "Sample Length", selectedKeys: selectedKeys, selectionMode: 'single', variant: 'flat', onSelectionChange: e => {
                                                if (!e.currentKey)
                                                    return;
                ...
                `, children: rangeLengthOptionsTags.map(option => (_jsx(DropdownItem, { children: `${textContent?.sampleLabelPrefix ?? 'Show'} ${option.value} ${textContent?.sampleLabelSuffix ?? 'entries'}` }, option.value))) })] })] }), _jsxs("div", { children: [_jsx("label", { htmlFor: "search", className: styleClassNames?.searchBar?.label ?? 'mr-2.5', children: textContent?.searchLabel ?? 'Search' }), _jsx("input", { type: "text", name: "search", id: "search", placeholder: textContent?.searchPlaceholder ?? '', className: `
              ...
              `, onChange: (e) => handleSearchInput(e) })] })] }), _jsx("div", { className: `
          ...
          `, children: _jsxs("table", { className: `w-full`, role: 'table', children: [_jsx("thead", { children: _jsx("tr", { role: 'row', className: `sticky top-0 z-10`, children: columns.map((key, index) => (_jsx("th", { role: 'columnheader', className: "cursor-pointer", ref: columnHeaderRef, children: _jsx("div", { "aria-label": `Sort by ${key.displayName}`, role: 'button', className: `
                      ...
                      `, onClick: e => handleOrderChange(e, key.property), children: _jsxs("div", { className: `flex items-center justify-between w-[100%] pl-[18px] pr-[5px]`, children: [_jsx("div", { className: 'flex-1 text-center overflow-hidden mr-[10px]', children: _jsx("p", { ref: columnNameRef, className: 'whitespace-nowrap', children: key.displayName ? key.displayName : String(key.property) }) }), _jsx("div", { children: _jsx("div", { className: `flex items-center justify-center w-[12px] h-[12px] mr-[10px]`, children: _jsx("svg", { className: `
                                ${!(activeOrder?.property === key.property) ? 'hidden' : ''}
                                ${activeOrder?.property === key.property && activeOrder?.order !== 'asc' ? 'rotate-90' : '-rotate-90'}
                              `, xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 320 512", fill: styleClassNames?.sortIndicatorColor ?? '#FFF', children: _jsx("path", { d: "M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z" }) }) }) })] }) }, index) }, uuidv4()))) }, uuidv4()) }), _jsxs("tbody", { className: 'pb-[2.5] overflow-y-auto ', children: [displayedRange.length === 0 && (_jsx("tr", { role: 'row', "data-row": true, className: `  
                    ${styleClassNames?.rows?.paddingB ?? ''}
                  `, ref: rowRef, children: _jsx("td", { role: 'cell', colSpan: columns.length, className: `
                    ...
                    `, children: _jsx("div", { className: `
                        ...
                        `, children: textContent?.emptyTableText ?? 'No data available in table' }) }, uuidv4()) }, uuidv4())), displayedRange.length > 0 && displayedRange.map((row, rowIndex) => (_jsx("tr", { ref: rowRef, role: 'row', "data-row": true, onMouseEnter: () => {
                                            setHoveredRow(row);
                                            if (onRowHover) {
                                                onRowHover(row);
                                            }
                  ...
                  `, children: columns.map((column, colIndex) => (Object.keys(row).includes(String(column.property)) && (_jsx("td", { title: row.id ? `id: ${row.id}` : '', role: 'cell', className: `
                        ...
                        `, children: _jsx("div", { className: `
                          ...
                          `, children: column.render
                                                    ...
                                                    : row[column.property] }) }, uuidv4())))) }, uuidv4())))] })] }) }), _jsxs("div", { hidden: pagesNumbers.length < 2, className: `flex flex-col lg:flex-row gap-y-3.5 justify-between items-center mt-5`, children: [_jsx("p", { className: 'inline-block', children: allRows.length > Math.min(rangeLength * currentPage, allRows.length)
                                ? (_jsxs("span", { children: [textContent?.rangeInfoText?.showEntries_altText ?? 'Showing entries', _jsx("span", { className: 'font-bold', children: rangeLength * (currentPage - 1) + 1 }), textContent?.rangeInfoText?.to_altText ?? 'to', _jsx("span", { className: 'font-bold', children: Math.min(rangeLength * currentPage, allRows.length) }), textContent?.rangeInfoText?.of_altText ?? 'of', _jsx("span", { className: 'font-bold', children: allRows.length }), textContent?.rangeInfoText?.entries_altText ?? 'entries'] }))
                                : (_jsxs("span", { children: [textContent?.rangeInfoText?.showEntries_altText ?? 'Showing entries', _jsx("span", { className: 'font-bold', children: rangeLength * (currentPage - 1) + 1 }), textContent?.rangeInfoText?.to_altText ?? 'to', _jsx("span", { className: 'font-bold', children: allRows.length }), textContent?.rangeInfoText?.entries_altText ?? 'entries'] })) }), _jsxs("div", { className: `
              flex items-center
            ...
            `, children: [currentPage - 1 >= 1 && (_jsx("button", { type: 'button', role: 'previousButton', onClick: () => setCurrentPage(currentPage - 1), className: `
                ...
                `, "aria-label": 'Previous page', children: _jsx("svg", { className: 'rotate-180 h-[15px]', xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 320 512", fill: styleClassNames?.pagination?.navButtonsColor ?? '#fff', children: _jsx("path", { d: "M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z" }) }) })), pagesNumbers.map((page, index) => typeof page === 'number' ? (_jsx("button", { onClick: () => setCurrentPage(page), className: `px-3 py-[5px] ${page === currentPage
                                        ? styleClassNames?.pagination?.currentPageButton ?? 'bg-blue-500 text-white first:rounded-l-[20px] last:rounded-r-[20px]'
                                        : styleClassNames?.pagination?.otherpages ?? 'text-white hover:bg-gray-500 first:rounded-l-[20px] last:rounded-r-[20px] cursor-pointer'}`, "aria-current": page === currentPage ? 'page' : undefined, children: page }, index)) : (_jsx("div", { className: 'flex items-start justify-start ', children: _jsx("span", { className: 'px-2 pb-2 text-white align-top', children: "..." }, index) }, index))), currentPage + 1 <= pagesNumber && (_jsx("button", { role: 'nextButton', "aria-label": 'Next page', onClick: () => setCurrentPage(currentPage + 1), className: `
                  disabled:opacity-50  cursor-pointer
                  ${styleClassNames?.pagination?.nextButtonPadding ?? 'pl-3 pr-[20px]'}
                  ${styleClassNames?.pagination?.nextButtonRoundedR ?? 'rounded-r-[20px]'}
                  ${styleClassNames?.pagination?.textColor ?? 'text-white'}
                  ${styleClassNames?.pagination?.buttonBackgroundColorHover ?? 'hover:bg-gray-500 py-[9px]'}
                `, children: _jsx("svg", { className: 'h-[15px]', xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 320 512", fill: styleClassNames?.pagination?.navButtonsColor ?? '#fff', children: _jsx("path", { d: "M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z" }) }) }))] })] })] }) }));
}
